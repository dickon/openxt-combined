...
static void get_xb_states(dominfo_t *domp, usbinfo_t *usbp, int *frontst, int *backst)
{
  char *bepath, *fepath;
  char *v;
  *frontst = XB_UNKNOWN;
...
static int test_offline(dominfo_t *domp, usbinfo_t *usbp)
{
  int f, b;
...
static void dump_dev(usbinfo_t *ui)
{
...
{
  fprintf(stderr, "usage: usbowls (assign|unassign|attach|detach|dump|list) -b <busid> -d <deviceid> -D <domid>\n");
  exit(1);
}
