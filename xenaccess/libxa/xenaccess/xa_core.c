...
#define VERSION_STR_LEN 100
    ...
    versions = xc_version(instance->m.xen.xch, cmd0, NULL);
    major = versions >> 16;
    ...
    xc_version(instance->m.xen.xch, cmd1, &extra);
    xa_dbprint("--extra = %s\n", (char *) extra);
...
/* given a xa_instance_t struct with the xch and the
...
int helper_init (xa_instance_t *instance)
{
    ...
    if (XA_MODE_XEN == instance->mode){
#ifdef ENABLE_XEN
        /* init instance->m.xen.xch */
        if (xc_domain_getinfo(
                instance->m.xen.xch, instance->m.xen.domain_id,
 ...
 * than the xch and the domain_id */
    ...
    (uint32_t domain_id, xa_instance_t *instance, uint32_t error_mode)
{
    ...
    if (!(xch = xc_interface_open(NULL, NULL, 0))){
        fprintf(stderr, "ERROR: Failed to open libxc interface\n");
        return XA_FAILURE;
    }
    instance->m.xen.xch = xch;
...
#ifdef ENABLE_XEN
    ret2 = xc_interface_close(instance->m.xen.xch);
#endif /* ENABLE_XEN */
